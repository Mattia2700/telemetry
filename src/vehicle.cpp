...
Fenice::Fenice() {
    ...
    // Initializing fenice protobuffer
    fenice_proto = new devices::Fenice();
    ...
    if(devices.size() != proto_messages.size()) {
        throw logic_error("Fenice initialization incorrect, vectors of devices and messages not of the same size");
    }

    // Get all names of the fenice message
    auto descriptor = fenice_proto->GetDescriptor();
    string separator = ";";
    for(int i = 0; i < descriptor->field_count(); i++) {
        string field = descriptor->field(i)->json_name();
        auto descriptor = proto_messages[i]->GetDescriptor();
...
void Fenice::add_filenames(string base_path, string extension) {
    for(auto device : devices) {
        device->filenames.push_back(base_path + "/" + device->get_name() + extension);
    }
}

void Fenice::open_all_files() {
    for(auto device : devices) {
        for(auto filename : device->filenames) {
            device->files.push_back(new std::fstream(filename, std::fstream::out));
        }
...
void Fenice::close_files(int index) {
    for(auto device : devices) {
        device->files[index]->close();
        device->files.erase(device->files.begin() + index);
        device->filenames.erase(device->filenames.begin() + index);
...
void Fenice::write_all_headers(int index) {
    for(auto device : devices) {
        *device->files[index] << device->get_header(",") << "\n" << flush;
    }
}

void Fenice::parse_message(const double& timestamp, const int& id, const uint8_t data[], const int& size, vector<Device*>& modifiedDevices) {
    modifiedDevices.clear();
    ...
    switch(id) {
        case 0x4EC:
            ...
            if(gyro->x > 32767) gyro->x -= 65536;
            if(gyro->y > 32767) gyro->y -= 65536;
            if(gyro->z > 32767) gyro->z -= 65536;
        ...
        case 0x4ED:
            ...
            if (accel->x > 32767) accel->x -= 65536;
            if (accel->y > 32767) accel->y -= 65536;
            if (accel->z > 32767) accel->z -= 65536;
        ...
        case 0xD0:
            ...
            encoder_left->rads /= 10000.0f;
            encoder_right->rads /= 10000.0f;
            if(data[6] == 1) encoder_left->rads *= -1;
            if(data[7] == 1) encoder_right->rads *= -1;
        ...
        case 0xAA:
            if(data[0] == 0x01) {
                ...
                bms_hv_state->value =
                    BMS_ERRORS[data[1]] + " -> " + to_string(data[2]);
                bms_hv_state->timestamp = timestamp;
                modifiedDevices.push_back(bms_hv_state);
            } else if (data[0] == 0x05) {
                bms_hv->current = ((data[1] << 8) + data[2]) / 10.0f;
        ...
        case 0x181:
            if (data[0] == 0xA0) {
                ...
                inverter_left->temperature =
                    ((data[2] << 8) + data[1] - 15797) / 112.1182;
                inverter_left->timestamp = timestamp;
                modifiedDevices.push_back(inverter_left);
            } else if (data[0] == 0x49) {
                inverter_left->motor_temp =
                    ((data[2] << 8) + data[1] - 9393.9) / 55.1;
                inverter_left->timestamp = timestamp;
                modifiedDevices.push_back(inverter_left);
            }
            // Speed filtered
                ...
                inverter_left->speed = (data[2] << 8) + data[1];
                if (inverter_left->speed > 32767) inverter_left->speed -= 65535;
        ...
        case 0x182:
            if (data[0] == 0xA0) {
                ...
                inverter_right->temperature =
                    ((data[2] << 8) + data[1] - 15797) / 112.1182;
                inverter_right->timestamp = timestamp;
                modifiedDevices.push_back(inverter_right);
            } else if (data[0] == 0x49) {
                inverter_right->motor_temp =
                    ((data[2] << 8) + data[1] - 9393.9) / 55.1;
                inverter_right->timestamp = timestamp;
                modifiedDevices.push_back(inverter_right);
            }
        ...
        case 0x55:
            if (data[0] == 0x01) {
                // if(size < NUM_STATES)
                //   break;
                ecu_state->value = "STATE: " + ECU_STATES[data[4]] +
                                   " Map: " + to_string(data[3]);
                ecu_state->timestamp = timestamp;
                modifiedDevices.push_back(ecu_state);
            } else if (data[0] == 0x0B && data[1] == 0x04) {
                ecu_state->value = "REQUEST TS OFF";
...
int Fenice::parse_gps(Gps* gps_, const double& timestamp, string& line) {
    UBX_MSG_PVT msg;
    if (parse_ubx_line(line, msg)) {
        gps_->clear();
        gps_->timestamp = timestamp;
    ...
    if (idx > 0) line = line.substr(idx, line.size() - idx);
    if (line[0] != '$') return -1;

    vector<string> s_line = split(line, ',');

    if (s_line[0].size() != 6) return -2;

    // remove $GP or $GN
    s_line[0].erase(0, 3);

    if (s_line[0] == "GGA") {
        if (s_line.size() != 15) return -6;
        // check if needed fileds are empty
        int ret = empty_fields(s_line, vector<int>{1, 2, 4, 6, 7, 9});
        if (ret != -1) return -3;
...
void Fenice::serialize() {
    this->accel->serialize(fenice_proto->add_accel());
    this->gyro->serialize(fenice_proto->add_gyro());

    this->encoder_left->serialize(fenice_proto->add_encoder_left());
    this->encoder_right->serialize(fenice_proto->add_encoder_right());

    this->bms_lv->serialize(fenice_proto->add_bms_lv());
    this->bms_hv->serialize(fenice_proto->add_bms_hv());

    this->inverter_left->serialize(fenice_proto->add_inverter_left());
    this->inverter_right->serialize(fenice_proto->add_inverter_right());

    this->pedal->serialize(fenice_proto->add_pedal());
    this->steer->serialize(fenice_proto->add_steer());

    this->ecu_state->serialize(fenice_proto->add_ecu_state());
    this->bms_hv_state->serialize(fenice_proto->add_bms_hv_state());
    this->steering_wheel_state->serialize(fenice_proto->add_steering_wheel_state());

    this->ecu->serialize(fenice_proto->add_ecu());

    this->temp_fl->serialize(fenice_proto->add_temp_fl());
    this->temp_fr->serialize(fenice_proto->add_temp_fr());
    this->temp_rl->serialize(fenice_proto->add_temp_rl());
    this->temp_rr->serialize(fenice_proto->add_temp_rr());

    this->gps1->serialize(fenice_proto->add_gps1());
    this->gps2->serialize(fenice_proto->add_gps2());
}

void Fenice::serialize_device(Device* device) {
    if (device == accel) {
        this->accel->serialize(fenice_proto->add_accel());
    } else if (device == gyro) {
        this->gyro->serialize(fenice_proto->add_gyro());
        ...
        this->steering_wheel_state->serialize(
            fenice_proto->add_steering_wheel_state());
    } else if (device == ecu) {
        this->ecu->serialize(fenice_proto->add_ecu());
    } else if (device == temp_fl) {
        this->temp_fl->serialize(fenice_proto->add_temp_fl());
